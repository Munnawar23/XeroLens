...
export const photoStorage = {
  savePhoto: async (uri: string): Promise<CapturedPhoto> => {
    try {
  ...
  getPhotoById: async (id: string): Promise<CapturedPhoto | undefined> => {
    try {
      const photos = await photoStorage.getPhotos();
      return photos.find((p) => p.id === id);
    } catch (e) {
      console.error("Error fetching photo by ID from MMKV:", e);
      return undefined;
    }
  },
